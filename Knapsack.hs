...
{-@ knapsack :: [(String,Int,Int)] -> x:{Int|x >= 0} -> (Int,[String]) @-}
knapsack :: [(String,Int,Int)] -> Int -> (Int,[String])
knapsack inv w = let l = foldr addItem (repeat (0,[])) inv in l !! w where
...
-- oh no, can't prove w is less than the length of an infinite list!
